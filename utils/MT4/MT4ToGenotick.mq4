...
input string   TODAY = "=== Ignore current bar by default (typically incomplete) ===";
input bool     IgnoreToday = true;
input string   OVERWRITE = "=== Overwrite existing data file? ===";
...
string exportBars() {
   ...
   if ( FileIsExist(filename) ) { 
      if ( Overwrite ) {
         ...
         return ("Data file " + filename + " aleady exists"); 
      }
   }
   ...
   if ( bars > 1 ) {
      ...
      for ( int i = bars-1; i >= (IgnoreToday?1:0); i-- ) {
